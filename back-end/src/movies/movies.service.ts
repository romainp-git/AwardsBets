...
export class MoviesService {
  ...
  async importFromTmdb(movieData: CreateMovieDto): Promise<Movie> {
    ...
    for (const personData of movieData.credits.cast.slice(0, 10)) {
      ...
      let person = await this.personRepository.findOne({ where: { name: personData.name } });
      let photo = `https://image.tmdb.org/t/p/h632${personData.profile_path}`;
  
      if (!person) {
        person = this.personRepository.create({ name: personData.name, photo: photo || undefined });
        await this.personRepository.save(person);
      }
  
      // 🔥 Création de l'association MoviePerson
      let moviePerson = this.moviePersonRepository.create({
        movie: savedMovie,
        person: person,
        roles: ['actor']
      });
    ...
    for (const personData of movieData.credits.crew) {
      if (Object.keys(VALID_DEPARTMENTS).includes(personData.department)) {
        ...
        let role = personData.job.charAt(0).toLowerCase() + personData.job.slice(1); // 🔹 Première lettre en minuscule

        let person = await this.personRepository.findOne({ where: { name: personData.name } });
        let photo = personData.profile_path ? `https://image.tmdb.org/t/p/h632${personData.profile_path}` : null;
