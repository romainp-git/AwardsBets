...
export class NomineesService {
  ...
  async updateOdds(nomineeId: number, updateOddsDto: UpdateOddsDto): Promise<Nominee> {
    const nominee = await this.nomineeRepository.findOne({ where: { id: nomineeId } });
  ...
  async setWinner(nomineeId: number) {
    const nominee = await this.nomineeRepository.findOne({ where: { id: nomineeId }, relations: ["category", "movie"] });
    if (!nominee) throw new NotFoundException("Nommé introuvable");

    const category = nominee.category;
    const previousWinner = await this.nomineeRepository.findOne({ where: { category: { id: category.id }, winner: true } });

    console.log("⚠️ Ancien gagnant détecté :", previousWinner ? previousWinner.id : "Aucun");
  ...
  async updateUsersScores() {
    ...
    const categories = await this.categoryRepository.find({ relations: ["nominees"] });
    const votes = await this.voteRepository.find({ relations: ["user", "nominee", "category"] });

    // 3️⃣ Parcours des catégories pour recalculer les scores
    for (const category of categories) {
        const winner = category.nominees.find(nominee => nominee.winner);
        if (!winner) {
            console.log(`⚠️ Pas de gagnant défini pour la catégorie ${category.name}, on passe.`);
            continue;
        }

        // Liste des nominés qui sont des losers
        const losers = category.nominees.filter(nominee => nominee.id !== winner.id);

        console.log(`🏆 Catégorie ${category.name}: Winner = ${winner.id}`);

        // 4️⃣ Attribution des points aux utilisateurs
        for (const user of users) {
            const userVotes = votes.filter(vote => vote.user.id === user.id && vote.category.id === category.id);

            let pointsGagnes = 0;

            // Vérifier s'il a trouvé le winner
            const hasWinner = userVotes.some(vote => vote.nominee.id === winner.id && vote.type === "winner");
            ...
            if (userLoserVote) {
                const loserNominee = losers.find(nominee => nominee.id === userLoserVote.nominee.id);
                if (loserNominee) {
  ...
  async notifyUsers(categoryName: string, nomineeName: string) {
    const users = await this.userRepository.find({ where: { pushToken: Not(IsNull()) } });
