...
const MovieDetailSheet: React.FC<MovieDetailSheetProps> = ({
  movie,
  onClose,
  cachedImages,
}) => {
  ...
  const director = useMemo(() => {
    const directorEntry = movie?.team.find((mp) =>
      mp.roles.includes("director")
    );
  ...
  useEffect(() => {
    ...
    const fetchListStatus = async () => {
      ...
      const isInWatchlist = await isMovieInList(
        Number(movie.id),
        ListType.WATCHLIST
      );
    ...
    >
      ...
      >
        ...
        <View className="flex-row justify-evenly space-x-6 mt-5">
          ...
          >
            <FontAwesome
              name={watched ? "eye" : "eye"}
              size={26}
              color={watched ? "green" : "#ccc"}
            />
            <Text
              className={`mt-1 ${watched ? "text-green-700" : "text-gray-300"}`}
            >
              Je l'ai vu
            </Text>
          </TouchableOpacity>
          ...
          >
            ...
            />
            <Text
              className={`mt-1 ${liked ? "text-red-700" : "text-gray-300"}`}
            >
              J'ai adoré
            </Text>
          </TouchableOpacity>
          ...
          >
            <FontAwesome
              name="plus-circle"
              size={26}
              color={watchlisted ? "dodgerblue" : "#ccc"}
            />
            <Text
              className={`mt-1 ${
                watchlisted ? "text-sky-700" : "text-gray-300"
              }`}
            >
              À Voir
            </Text>
        ...
        {movie.team?.length > 0 && (
          <View className="mt-5">
            <Text className="px-5 text-white text-lg font-semibold mb-2">
              Cast
            </Text>
            <FlatList
              horizontal
              showsHorizontalScrollIndicator={false}
              data={movie.team.filter(
                (member) =>
                  member.roles.includes("actor") &&
                  member.person.photo !== "https://image.tmdb.org/t/p/h632null"
              )}
              ...
              renderItem={({ item }) => (
                <View className="items-center mr-4 w-20">
                  <ExpoImage
                    source={{
                      uri: cachedImages[item.person.photo] || item.person.photo,
                    }}
