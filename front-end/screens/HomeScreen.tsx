...
import Animated, { interpolate, useAnimatedScrollHandler, useAnimatedStyle, useDerivedValue, useSharedValue, withTiming } from 'react-native-reanimated';

import { useData } from "../data/data";
import { Movie } from "../types/types";

export default function HomeScreen({ cachedImages }: { cachedImages: { [key: string]: string } }) {
  const insets = useSafeAreaInsets();
  const headerHeight = 420;
  const { competition, categories, movies, loading, error } = useData();
  ...
  const animatedCarouselStyle = useAnimatedStyle(() => {
    return {
      height: withTiming(Math.max(0, headerHeight - scrollY.value), { duration: 0 }), // ⚡ ZÉRO latence
    };
  });
  ...
  return (
    <View className="bg-zinc-900 flex-1 relative">
      ...
      >
        {(topNominatedMovies.length && Platform.OS === "android") ? (
          <Animated.View style={animatedCarouselStyle}>
            <NomineeCarousel movies={topNominatedMovies} />
          </Animated.View>
        ) : null}

        <Image
          source={require("../assets/oscars-logo.png")}
          style={{
            width: "auto",
        ...
        <View className="flex flex-col justify-between items-center px-4 pb-4">
          <Text className="text-[#B3984C] tracking-wide text-sm" style={{ fontFamily: "FuturaBook" }}>{competition?.edition.toUpperCase()}</Text>
          <Text className="text-white text-xl" style={{ fontFamily: "FuturaHeavy" }}>
            {competition?.date.toLocaleDateString("fr-FR", formatDateOptions).replace(/\b\w/g, (char) => char.toUpperCase())}
          </Text>
          <Text className="text-[#B3984C] tracking-wide text-sm" style={{ fontFamily: "FuturaBook" }}>{competition?.venue.toUpperCase()} - {competition?.city.toUpperCase()}</Text>
        </View>
      ...
      {(topNominatedMovies.length && Platform.OS === "ios") ? (
        <Animated.View style={[animatedCarouselStyle, { position: "absolute", top: 0, left: 0, right: 0 }]}>
          <NomineeCarousel movies={topNominatedMovies} />
        </Animated.View>
      ) : null}
